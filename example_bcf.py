...
# Uses numba-accelerated kernels from cbf_numba_lib.py.
...
# Import numba-accelerated helpers
...
v_h_default = 1.6  # [m/s]  assumed human approach speed (not used directly here)
...
def main():
    ...
    planner.addWayPoint(goal_pose * pose_eul(math.pi / 4, 0.0, 0.0, [0.30, -0.1, 0.020]))  # 45° Z while moving
    planner.addWayPoint(goal_pose * pose_eul(math.pi / 4, 0.0, -math.pi / 4, [0.3, -0.1, 0.2]))  # add Y/X tilt
    planner.addWayPoint(goal_pose * pose_eul(-math.pi / 4, 0.0, 0.0, [0.30, 0.0, 0.0]))  # spin back + arc
    planner.addWayPoint(goal_pose * SE3.Identity())  # return to start
    ...
    try:
        ...
        while t < 150.0:
            ...
            for i, obs_pos in enumerate(obstacle_positions):
                ...
                v_h_scalar = np.dot(u_hr, v_obs)
                v_rel = np.dot(vel_lineare, u_hr)

                # h and its dynamics/derivatives via numba kernels
                ...
                constraint_matrix = np.append(
                    constraint_matrix, (Lie_g_h @ Jlin).reshape(1, -1), axis=0
                )
            ...
            if CBF:
                ...
                # no CBF, ddq calculated based on dtwist_tool:
                # dtwist_tool (Cartesian acc) = J @ ddq + dJ @ dq
