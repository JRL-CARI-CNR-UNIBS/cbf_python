"""Obstructive Obstacle Injection Test runner."""
...
def main(config_file: str = "run.yaml") -> None:
    config = load_run_config("obstructive_test", config_file)
    ...
    spawner_cfg = config.get("obstacle_spawner", {})
    spawn_freq = int(spawner_cfg.get("spawn_freq", 500))
    d_objective = float(spawner_cfg.get("d_objective", 0.25))
    v_ref = float(spawner_cfg.get("v_ref", 0.1))
    enable_spawn = bool(spawner_cfg.get("enable_spawn", True))
    ...
    obs_positions = np.array([[0.0, 0.0, 0.0]])
    obs_velocities = np.array([[0.0, 0.0, 0.0]])
    obs_accelerations = np.array([[0.0, 0.0, 0.0]])
    ...
    print(f"Starting Obstructive Test Control Loop for duration = {duration}s...")
    while t < duration and not stop_event.is_set():
        ...
        p_ee, _, _ = compute_ee_pose(q, model, data, tool_frame_id)
        twist_ee = pin.getFrameVelocity(model, data, tool_frame_id, pin.ReferenceFrame.LOCAL_WORLD_ALIGNED)

        obs_positions, obs_velocities, enable_spawn, count_move = generate_obs_state_h_fixed(
        ...
        dtraj_time = out["Dtrajectory_time"]
        cycles += 1

        if viz_daemon:
            hud_text = f"Scaling: {out['Dtrajectory_time']:.2f}\nh_min: {out['h_min']:.3f}m"
            viz_daemon.push_state(q=q, Tgoal=out["Tbt_nominal"], obstacles=[p for p in obs_positions], obstacle_velocities=[v for v in obs_velocities], viz_string=hud_text)
