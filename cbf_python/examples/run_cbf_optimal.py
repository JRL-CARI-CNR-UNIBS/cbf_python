"""Optimal CBF Controller simulation and hardware execution runner."""
...
def main(config_file: str = "run.yaml") -> None:
    config = load_run_config("cbf_optimal", config_file)
    ...
    # 4. Optional Data Loggers
    loggers: Dict[str, Any] = {}
    if log_data:
        now_str = datetime.now().strftime("%Y%m%d_%H%M%S")
        test_dir = os.path.join(log_path, now_str)
        os.makedirs(test_dir, exist_ok=True)
        if use_bridge:
            loggers["joint_target"] = JointTargetPublisher(topic="joint_target", joint_names=UR10E_JOINTS)
            loggers["test_start"] = TestStartPublisher(topic="test_start")
            loggers["cbf_out"] = DoubleArrayPublisher(topic="cbf_output")
            loggers["human_pos"] = DoubleArrayPublisher(topic="human_pos_keypoints")
        else:
            loggers["joint_target"] = JointTargetCsvPublisher(
                csv_path=os.path.join(test_dir, "reference_trajectory.csv"),
                joint_names=UR10E_JOINTS,
            )
            loggers["joint_state"] = JointTargetCsvPublisher(
                csv_path=os.path.join(test_dir, "joint_states.csv"),
                joint_names=UR10E_JOINTS,
            )
            loggers["test_start"] = TestStartCsvPublisher(
                csv_path=os.path.join(test_dir, "TEST_START.csv"),
            )
            loggers["cbf_out"] = DoubleArrayCsvPublisher(
                csv_path=os.path.join(test_dir, "cbf_results.csv"),
            )
            loggers["human_pos"] = DoubleArrayCsvPublisher(
                csv_path=os.path.join(test_dir, "human_positions.csv"),
            )
    ...
    if viz_daemon:
        path_pts = []
        for s in np.linspace(0, T_total, 300):
            q_nom, _, _ = planner.getMotionLaw(s)
            p_ee, _, _ = compute_ee_pose(q_nom, model, data, model.getFrameId(target_name))
            path_pts.append(p_ee)
        viz_daemon.publish_path(np.array(path_pts))
    ...
    print(f"Starting CBF Optimal Control Loop for duration = {duration}s...")
